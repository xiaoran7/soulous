...
> AI 驱动的全栈游戏化学习闭环：AI 拆解对话 → 任务落地 → 自习室专注 → 凭证提交 → 多模型 AI 审核 → RAG 长时记忆 → 经验奖励 → 虚拟宠物成长。
...
- **AI 拆解对话**：Gemini 式聊天（分类 → 对话 → 消息，可收起侧边栏、对话归类/移动）、流式回复、上传 md/pdf/txt 喂资料、AI 给出可编辑计划草案后一键落地为任务、任务生命周期管理
- **自习室 / 凭证**：自习室正计时专注（选场景 + 环境音/音乐 + 全屏沉浸，可自定义上传场景图/音乐）、图文 / 代码 / 截图凭证提交（白名单 + 5MB 限制 + 自动压缩 1920px / JPEG 85%）、鉴权下载 `/uploads/**`
- **课表**：导入教务系统学期课表（`.xls` 浏览器内 SheetJS 解析 → LLM 结构化，或粘贴 HTML）、按学期/周次（含单双周）展示、手动增删、开学日期定位本周
- **AI 中枢**：
  - AI 拆解对话（连续聊天 + `PLAN_JSON` 计划草案落地为任务）
...
backend/                 Spring Boot + JPA API
  src/main/java/com/soulous/
    chat/                AI 拆解对话（分类/对话/消息，Gemini 式）
    aisession/ goal/     旧目标拆解（已下线，表/服务暂留供 RAG/复盘引用）
...
`POST /api/chat/conversations/{id}/messages/stream` 走 `text/event-stream`，emit 三种事件：

- `event: token` — 增量文本片段，`data` 为 JSON 编码的字符串
- `event: done` — 流结束，`data` 为完整 ConversationView JSON（含 pendingPlan / pendingClarify / messages）
